...
BOOST_AUTO_TEST_CASE(validate_complex_object_valid)
{
    ...
    BOOST_CHECK_NO_THROW(validate_complex_type(person, person_def, defs));
}

// Test 6: Validate complex object - missing required field
BOOST_AUTO_TEST_CASE(validate_complex_object_missing_field)
{
    ...
    BOOST_CHECK_EXCEPTION(
        validate_complex_type(person, person_def, defs),
        std::runtime_error,
        [](const std::runtime_error& e) {
            return std::string(e.what()).find("Missing required component 'age'") != std::string::npos;
        }
    );
}
...
BOOST_AUTO_TEST_CASE(validate_nested_complex_object)
{
    ...
    BOOST_CHECK_NO_THROW(validate_complex_type(customer, customer_def, defs));
}

// Test 8: Validate nested object - missing nested field
BOOST_AUTO_TEST_CASE(validate_nested_object_missing_nested_field)
{
    ...
    BOOST_CHECK_EXCEPTION(
        validate_complex_type(customer, customer_def, defs),
        std::runtime_error,
        [](const std::runtime_error& e) {
            return std::string(e.what()).find("Missing required component 'city'") != std::string::npos;
        }
    );
}
...
BOOST_AUTO_TEST_CASE(validate_collection_of_complex_objects)
{
    ...
    BOOST_CHECK_NO_THROW(validate_complex_type(order, order_def, defs));
}

// Test 10: Validate collection - invalid element
BOOST_AUTO_TEST_CASE(validate_collection_invalid_element)
{
    nlohmann::json order = {
        ...
        {"items", nlohmann::json::array({
            {{"product", "Widget"}, {"quantity", 5}},
            {{"product", "Gadget"}}  // Missing quantity
        })}
    };
    ...
    BOOST_CHECK_EXCEPTION(
        validate_complex_type(order, order_def, defs),
        std::runtime_error,
        [](const std::runtime_error& e) {
            std::string msg = e.what();
            return msg.find("items[1]") != std::string::npos &&
                   msg.find("quantity") != std::string::npos;
        }
    );
}
...
BOOST_AUTO_TEST_CASE(validate_component_with_constraints)
{
    ...
    BOOST_CHECK_NO_THROW(validate_complex_type(employee, employee_def, defs));
}
...
BOOST_AUTO_TEST_CASE(validate_component_constraint_violation)
{
    ...
    BOOST_CHECK_EXCEPTION(
        validate_complex_type(employee, employee_def, defs),
        std::runtime_error,
        [](const std::runtime_error& e) {
            std::string msg = e.what();
            return msg.find("role") != std::string::npos &&
                   msg.find("CEO") != std::string::npos;
        }
    );
}
...
BOOST_AUTO_TEST_CASE(validate_collection_not_array)
{
    ...
    BOOST_CHECK_EXCEPTION(
        ...
        [](const std::runtime_error& e) {
            std::string msg = e.what();
            return msg.find("items") != std::string::npos &&
                   msg.find("array") != std::string::npos;
        }
    );
}
...
BOOST_AUTO_TEST_CASE(validate_not_an_object)
{
    ...
    BOOST_CHECK_EXCEPTION(
        validate_complex_type(not_object, person_def, defs),
        std::runtime_error,
        [](const std::runtime_error& e) {
            return std::string(e.what()).find("Expected object") != std::string::npos;
        }
    );
}
