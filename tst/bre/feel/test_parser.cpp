...
using orion::bre::EvaluationContext;  // EvaluationContext moved to parent namespace

/**
 * Helper function to parse and evaluate an expression
 */
json parse_and_evaluate(const std::string& expression, const json& input)
{
    Lexer lexer;
...
{
    json input = {{"age", 25}};
    auto result = parse_and_evaluate("age", input);
    BOOST_CHECK(result.is_number_integer());
...
{
    json input = {{"Monthly Salary", 10000}};
    auto result = parse_and_evaluate("Monthly Salary", input);
    BOOST_CHECK(result.is_number_integer());
...
{
    json input = {{"Monthly_Salary", 10000}};
    auto result = parse_and_evaluate("Monthly Salary", input);
    BOOST_CHECK(result.is_number_integer());
    BOOST_CHECK_EQUAL(result.get<int>(), 10000);
}
...
{
    json input = {};
    BOOST_CHECK_THROW(parse_and_evaluate("undefined_var", input), std::runtime_error);
}
...
{
    json input = {{"age", 25}, {"priority", 7}};
    auto result = parse_and_evaluate("age >= 18 and priority > 5", input);
    BOOST_CHECK(result.is_boolean());
    BOOST_CHECK_EQUAL(result.get<bool>(), true);
}
...
{
    json input = {{"salary", 10000}, {"bonus", 2000}};
    auto result = parse_and_evaluate("(salary + bonus) * 12", input);
    BOOST_CHECK(result.is_number());
...
BOOST_AUTO_TEST_CASE(TestParseDMNLikeExpression)
{
    ...
    auto result = parse_and_evaluate("Monthly Salary - Monthly Expenses > 5000", input);
    BOOST_CHECK(result.is_boolean());
