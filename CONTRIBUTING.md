...
## Code-Focused AI Policy

**ORION is a 100% AI-Generated Project.**

To maintain the integrity and style of the codebase, we enforce the following policy:

1.  **Source Code**: All changes to source code (`src/`, `include/`, `tst/`) **MUST** be generated by an AI agent (e.g., GitHub Copilot, Claude, ChatGPT) based on a detailed task prompt.
2.  **Manual Edits**: Manual edits are permitted **ONLY** for:
    *   Documentation (`*.md`, `docs/`)
    *   Configuration files (`CMakeLists.txt`, `.github/`, `vcpkg.json`)
    *   Trivial fixes (typos, formatting)
3.  **Verification**: You must certify in your Pull Request that the code was AI-generated.

### AI-First Workflow

1.  **Create an Issue**: Describe the bug or feature request.
2.  **Create a Task File**: Create a new markdown file in `.github/tasks/` named `<ISSUE_NUMBER>_<snake_case_title>.md`.
    *   Use the templates in `.github/prompts/` if applicable (e.g., `add_dmn_feature.md`, `fix_bug.md`).
    *   Define the Context, Scope, and Success Criteria.
3.  **Execute with AI**: Use an AI coding agent to execute the task defined in the file.
4.  **Retrospective**: After completion, the agent (or you) should fill in the "Retrospective" section of the task file.
5.  **Submit PR**: Link the task file in your Pull Request description.
